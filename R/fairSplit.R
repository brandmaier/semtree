fairSplit <- function(model=NULL, mydata=NULL, control=NULL, invariance=NULL, meta=NULL,
                      constraints=NULL, ...) {
  ...
  cross2 <- subset (mydata, as.numeric(mydata[,ncol(mydata)])<= 0.50)
  ...
  #    message("Testing ",c,"/",ncol(cross1), " (",colnames(cross1)[c],")" )
        ...
        if(length(val.sets) > 1) {
          ...
          for(i in 1:(result$num_sets)) {
            ...
            if (!is.null(constraints) & (!is.null(constraints$focus.parameters))) {
              ...
              if (control$report.level > 10) {
                report(paste("Reestimating baseline likelihood with focus parameters: ",
                             LL.baseline),2)
              }
            }
    ...
    if (control$report.level>0) {
      if (is.null(LL.within)) {
        report("No valid estimate found for any split value during the first round.",2)
      }
    }
    ...
    if (!is.null(LL.within)) {
      ...
      if (control$report.level >= 3) {
        report(paste("Best split at ",max.within.split," with LL",max.LL.within),2)
      }
  ...
  if(!is.null(LL.btn)) {
    for(c in 1:length(LL.btn)) {
      ...
      # refit baseline model with focus parameters @TAGX
      if (!is.null(constraints) & (!is.null(constraints$focus.parameters))) {
        result <- fitSubmodels(model, subset1, subset2, 
                                    control, invariance=constraints$focus.parameters,
                                    return.models=TRUE)
        if (control$report.level > 10) {
      ...
      # evaluate split  
      LL.cur <- LL.baseline - fitSubmodels(model, subset1, subset2, control, invariance=NULL)
      # browser()
  ...
  btn.matrix <- rbind(LL.btn, names(mydata[c(cov.btn.cols)]),cov.btn.cols,split.btn)
  ...
  if(control$report.level>=2) {
    ...
    report(paste("Best predictor", name.max,"split at",split.max, "with LL",LL.max),1)
    report("",1)
  }
