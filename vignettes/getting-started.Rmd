...
We first load the `semtree` package and both the `OpenMx` and `lavaan` packages for specifying our SEM.
...
Now, we specify a linear latent growth curve model using lavaan. The model has five observed variables. Residual variances are assumed to be identical over time.
...
"
growthCurveModel <- lavaan::growth(
  model = model,
  data = growth.data,
  do.fit = FALSE
)
